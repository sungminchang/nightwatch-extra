...
module.exports = function statd(event) {
  ...
  client.gauge(fullEventKey + ".totalTime", event.value.totalTime);
  client.gauge(fullEventKey + ".seleniumCallTime", event.value.seleniumCallTime);
  client.gauge(fullEventKey + ".executeAsyncTime", event.value.executeAsyncTime);
};
